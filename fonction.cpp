    ...
    //////////////////////////////////////////////////////////////
        ...
        while(getline(fichier, ligne))
        {
           ...
           // cout << "\t" << TabDesignation.size() << endl;
            ...
            //remplissage TabDasignation2 à afficger
            bool cond = false; bool cond_remp = false; int n = 1;
            for (int i=1; i<TabDesignation; i++)
            {
                if (atoi(TabDesignation[i][2].c_str())==n)//si on est sur la même page
                {
                    if (cond == true)
                    {
                        TabDesignation2[TabDesignation2.size()-1].push_back(TabDesignation[i][12]);//création des colonnes du tableau à afficher
                    }
                    else if (TabDesignation[i-1][11] == "-1" && TabDesignation[i][12] == "DATE")
                    {
                        TabDesignation2.push_back(vector<string>(1,TabDesignation[i][12]));
                        cond = true; cond_remp = true;
                    }
                    else if (TabDesignation[i][11] == "-1")
                    {
                        cond = false;
                    }
                    else if (cond_remp == true)
                    {
                        //remplir TabDesignation2 par TabDesignation[i][12] en revenant à la ligne si -1
                    }

                }

                if (TabDesignation[i][12] == "Page")
                {
                    n += 1; //on passe à une nouvelle page
                }
            }

            //afficher nouveau tableau créé
            for (int i=0; i<TabDesignation2.size(); i++)
            {
                for (int j=0; j<TabDesignation2[i].size(); j++)
                {
                    cout << TabDesignation2[i][j] << "\t";
                }
                cout << "\t" << TabDesignation2.size() << endl;
            }
